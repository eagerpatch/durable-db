...
export function generateRpcStubs(
  ...
  const stubs = actions.map(action => {
    ...
    // Generate the validator for this action's args
    const validatorName = `${action.exportName}ArgsValidator`;

    return `
const ${validatorName} = type(${action.argsSchemaSource});

/**
 * ${action.exportName} action
 * @generated by @shoplayer/database
 */
export async function ${action.exportName}(args: typeof ${validatorName}.infer) {
  // Validate args before RPC call
  const validatedArgs = ${validatorName}.assert(args);
  
  const ctx = getContext();
  const env = ctx.env;
  const id = ${idStrategy};
  const stub = env.${database.bindingName}.get(id);
  return stub.${action.exportName}(validatedArgs, { instanceKey: ${instanceKey} });
}`;
  }).join('\n');
